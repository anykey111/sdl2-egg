(use sdl2-defs sdl2-basics sdl2-events sdl2-video sdl2-ttf)

(define running  (make-parameter #t))
(define window)
(define render)
(define font)
(define font-surface)
(define font-texture)
(define img1)

(define (gltest)
  (let ((glwindow (sdl-create-window "gl test" 500 500 320 240 (+ SDL-WINDOW-OPENGL SDL-WINDOW-SHOWN))))
    (sdl-gl-set-attribute SDL-GL-CONTEXT-MAJOR-VERSION 3)
    (sdl-gl-set-attribute SDL-GL-CONTEXT-MINOR-VERSION 2)
    (sdl-gl-set-attribute SDL-GL-DOUBLEBUFFER 2)
    (let ((glcontext (sdl-gl-create-context glwindow)))
      (sdl-gl-make-current glwindow glcontext)
      (sdl-gl-set-swap-interval 1)
      (sdl-gl-get-swap-interval)
      (print "glwindow:" (sdl-gl-get-current-window))
      (print "glcontext:" (sdl-gl-get-current-context))
      (let loop ((e (sdl-poll-event)))
        ;(sdl-gl-bind-texture tex1 1. 1.)
        ;(sdl-gl-unbind-texture tex1)
        (sdl-gl-swap-window glwindow)
        (unless (and e (= SDL-QUIT (sdl-event-type e)))
          (loop (sdl-poll-event))))
      (sdl-gl-delete-context glcontext)
      (sdl-destroy-window glwindow))))

(define (main-loop)
  (let ((e (sdl-poll-event)))
    (if e (handle-event e)))
  (sdl-set-render-draw-color render 0 0 0 255)
  (sdl-render-clear render)
  (sdl-set-render-draw-color render 255 0 0 255)
  (sdl-render-draw-line render 10 10 100 100)
  (sdl-render-draw-lines render
    (make-sdl-point 100 100)
    (make-sdl-point 100 200)
    (make-sdl-point 200 100)
    (make-sdl-point 200 200))
  (sdl-render-draw-point render 300 300)
  (sdl-render-draw-points render
    (make-sdl-point 299 300)
    (make-sdl-point 301 300)
    (make-sdl-point 300 299)
    (make-sdl-point 300 301))
  (sdl-render-draw-rect render (make-sdl-rect 295 295 50 50))
  (sdl-render-fill-rect render (make-sdl-rect 305 305 5 8))
  (sdl-render-copy render font-texture #f (make-sdl-rect 300 100 50 50))
  (sdl-render-copy render tex1 #f (make-sdl-rect 100 100 50 50))
  (sdl-render-present render)
  (if (running)
    (main-loop)))

(define (handle-event e)
  (sdl-log "event:" e)
  (select (sdl-event-type e)
    ((SDL-QUIT)
     (sdl-log "quit:")
     (running #f))
    ((SDL-KEYUP SDL-KEYDOWN)
     (sdl-log "key:" (sdl-event-user e)))))

(define (main)
  (sdl-log (sdl-version))
  (sdl-init (+ SDL-INIT-VIDEO))
  (gltest)
  (ttf-init)
  (set! font (ttf-open-font "lucon.ttf" 20))
  (set! window (sdl-create-window "sdl2 test" 100 100 640 480 SDL-WINDOW-OPENGL))
  (set! render (sdl-create-renderer window))
  (set! font-surface (ttf-render-text-solid font "FIGA" (make-sdl-color 0 255 0 255)))
  (set! font-texture (sdl-create-texture-from-surface render font-surface))
  (set! img1 (sdl-load-bmp "1.bmp"))
  (set! tex1 (sdl-create-texture-from-surface render img1))
  (main-loop)
  (sdl-free-surface img1)
  (sdl-destroy-texture tex1)
  (ttf-quit)
  (sdl-quit))

(main)
